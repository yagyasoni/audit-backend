...
export const insertInventoryRows = async (auditId, rows) => {
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i += CHUNK_SIZE) {
      ...
      console.log(`✅ Chunk ${Math.floor(i / CHUNK_SIZE) + 1}/${Math.ceil(rows.length / CHUNK_SIZE)} inserted`);
    }
    ...
    try {
      ...
      for (let i = 0; i < records.length; i += CHUNK_SIZE) {
        ...
        const placeholders = chunk.map((row, idx) => {
          ...
          const invoiceDate = mapping.invoiceDate ? (row[mapping.invoiceDate] ?? null) : null;
          const productName = mapping.itemDescription ? (row[mapping.itemDescription] ?? null) : null;
          const quantity = mapping.quantity && row[mapping.quantity] !== undefined && row[mapping.quantity] !== ""
            ? (parseInt(String(row[mapping.quantity]).replace(/[^0-9-]/g, "")) || 0)
            : null;
          const unitCost = mapping.unitPrice && row[mapping.unitPrice]
            ? parseFloat(String(row[mapping.unitPrice]).replace(/[^0-9.]/g, "")) : null;
          const totalCost = mapping.totalPrice && row[mapping.totalPrice]
            ? parseFloat(String(row[mapping.totalPrice]).replace(/[^0-9.]/g, "")) : null;
        ...
        console.log(`✅ Wholesaler chunk ${Math.floor(i / CHUNK_SIZE) + 1}/${Math.ceil(records.length / CHUNK_SIZE)} inserted for ${fileObj.wholesaler_name}`);
      }

      await client.query("COMMIT");
      console.log(`✅ All ${records.length} wholesaler rows inserted for ${fileObj.wholesaler_name}`);
    } catch (err) {
      await client.query("ROLLBACK");
      throw err;
    } finally {
...
export const getAuditById = async (auditId) => {
  const result = await pool.query(`SELECT * FROM audits WHERE id = $1`, [auditId]);
  return result.rows[0] || null;
};
...
export const getInventoryRows = async (auditId) => {
  ...
  const REQUIRED_COLUMNS = [
    "ndc", "rx_number", "status", "date_filled", "drug_name",
    "quantity", "package_size", "primary_bin", "primary_paid",
    "secondary_bin", "secondary_paid", "brand",
  ];
